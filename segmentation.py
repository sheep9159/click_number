...
def read_eeg_file(file_dir, point_boundary, name_index, trail_indices):
        ...
        with open(file_dir, 'r') as f:
            for line in f:
                if row <= 5 and row != 3:
                    txt = txt + line
                if row == 3:
                    current_name_index = line[9:-5] + '{0:0>2}'.format(trail_index + 1)  # 控制编号以01， 02，形式
...
def read_vhdr_file(file_dir, name_index, trail_indices, datas_length):
    row = 0  # 用来记录读到第几行了
    txt = ''  # 先创建一个空字符串，用来存待会改写后的文本
    for trail_index in range(trail_indices):
        current_name_index = name_index[trail_index]
        ...
        f = open(data_dir.segmentation_dir + '\\raw_data' + '\\' + name_index[trail_index] + '.vhdr', 'w')
        f.write(txt)
        f.close()
        txt = ''
...
if __name__ == '__main__':
    ...
    for i in range(len(eeg_file_name)):
        ...
        point_boundary, name_index, trail_indices, datas_length = read_mark_txt(mark_file_name[i])
        read_vhdr_file(vhdr_file_name[i], name_index, trail_indices, datas_length)
        read_eeg_file(eeg_file_name[i], point_boundary, name_index, trail_indices)
